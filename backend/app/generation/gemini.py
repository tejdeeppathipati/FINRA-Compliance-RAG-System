...
def parse_generated_payload(
    ...
    """Parse model JSON and reject any citation that is not in retrieved evidence."""
    try:
        payload = json.loads(response_text or "")
    except (TypeError, json.JSONDecodeError) as error:
        raise GenerationProviderError("Model did not return valid JSON") from error
    if not isinstance(payload, dict) or not isinstance(payload.get("answer"), str):
        raise GenerationProviderError("Model response is missing an answer")
    abstained = bool(payload.get("abstained"))
    raw_citations = payload.get("citations", [])
    if not isinstance(raw_citations, list):
        raise GenerationProviderError("Model citations are not a list")
    if abstained:
    ...
    for raw in raw_citations:
        if not isinstance(raw, dict):
            raise GenerationProviderError("Model returned an invalid citation")
        ...
        if not matches or not isinstance(excerpt, str) or excerpt not in matches[0].content:
            raise GenerationProviderError("Model returned a citation not supported by evidence")
        citations.append(
            {
    ...
    if not citations:
        raise GenerationProviderError("Model returned no citations for a non-abstained answer")
...
def generate_grounded_answer(
    ...
    *,
    ...
    return parse_generated_payload(response.text, passages)
